...
import logger.CRUD;
import logger.LoggingHelper;
import logger.Operation;
...
public class MainActivity implements Activity, ProductsView.Presenter {
    ...
    private int getRowTable(String productId){
        ...
        for(int i = 0; i < jTable.getRowCount(); i++){
                //For each row
                if(jTable.getModel().getValueAt(i, 0).equals(productId)){ //Search the model
    ...
    private void checkYourItem() {
        ...
        productPrice = iValidator.isFloat(str, 1000F);
